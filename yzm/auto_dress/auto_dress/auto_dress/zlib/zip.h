...
ZRESULT ZipAddHandle(HZIP hz,const char *dstzn, FILE* h);
ZRESULT ZipAddHandle(HZIP hz,const char *dstzn, FILE* h, unsigned int len);
...
//     FILE* hread,hwrite; CreatePipe(&hread,&hwrite,NULL,0);
//     FILE* hthread = CreateThread(0,0,ThreadFunc,(void*)hwrite,0,0);
//     ZipAdd(hz,"unz3.dat",  hread,1000);  // the '1000' is optional.
//     WaitForSingleObject(hthread,INFINITE);
//     CloseHandle(hthread); CloseHandle(hread);
//     ... meanwhile unsigned long WINAPI ThreadFunc(void *dat)
//                   { FILE* hwrite = (FILE*)dat;
//                     char buf[1000]={17};
//                     unsigned long writ; WriteFile(hwrite,buf,1000,&writ,NULL);
//                     CloseHandle(hwrite);
//                     return 0;
//                   }
//     // and now that the zip is created, let's do something with it:
//     void *zbuf; unsigned long zlen; ZipGetMemory(hz,&zbuf,&zlen);
//     FILE* hfz = CreateFile("test2.zip",GENERIC_WRITE,0,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);
//     unsigned long writ; WriteFile(hfz,zbuf,zlen,&writ,NULL);
//     CloseHandle(hfz);
//     CloseZip(hz);
//
// (3) Handle use, for file handles and pipes
//     FILE* hzread,hzwrite; CreatePipe(&hzread,&hzwrite,0,0);
//     FILE* hthread = CreateThread(0,0,ZipReceiverThread,(void*)hzread,0,0);
...
//     WaitForSingleObject(hthread,INFINITE);
//     CloseHandle(hthread);
//     ... meanwhile unsigned long WINAPI ZipReceiverThread(void *dat)
//                   { FILE* hread = (FILE*)dat;
//                     char buf[1000];
//                     while (true)
//                     { unsigned long red; ReadFile(hread,buf,1000,&red,NULL);
